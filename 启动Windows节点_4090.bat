@echo off
rem Keep this file pure ASCII. RTX 4090 low-VRAM launcher.
chcp 65001 >nul
cd /d "%~dp0"
...
set "COMFY_PORT=8188"
set "RESERVE_VRAM=3"
set "COMFY_EXTRA_ARGS=--windows-standalone-build --lowvram"
set "START_DELAY=8"
...
if /i "%~1"=="check" (
  echo Windows RTX 4090 node launcher paths are valid.
  exit /b 0
)

"%PY%" -c "import socket,sys; s=socket.socket(); s.settimeout(1); sys.exit(s.connect_ex(('%COMFY_HOST%',%COMFY_PORT%)) != 0)"
if errorlevel 1 (
  echo Starting ComfyUI in RTX 4090 low-VRAM mode...
  start "ComfyUI RTX 4090" cmd /k ""%PY%" -s "%COMFY_MAIN%" %COMFY_EXTRA_ARGS% --listen %COMFY_HOST% --port %COMFY_PORT% --reserve-vram %RESERVE_VRAM%"
...
echo.
echo Windows RTX 4090 node started. Keep the ComfyUI and Worker log windows open.
pause
