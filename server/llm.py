...
RESERVE = 2048      # max_tokens 之外额外给的额度。**思考模型的思考段也算在
                    # max_tokens 里**：h3_ref 给 2400，思考先吃掉约 2000，正文
                    # 只剩 1399 字、截在半句话上（网关把思考放进 reasoning_content，
                    # 正文看着是干净的 —— 所以这个坑不看 usage 根本发现不了）。
                    # REGIMES 那几个数是照本地 27B 的 n_ctx 定的，那边不含思考段，
                    # 不能直接拿来当 API 的上限。
                    #
                    # 只加不改 REGIMES：max_tokens 是**上限不是目标**，不思考的模型
                    # 写完就停，多给的额度一个 token 都不会用到、也不多花钱。
                    # 唯一的副作用是老模型（输出上限 4096 那种）可能回 HTTP 400，
                    # 那种情况 HINT[400] 已经提了模型名和 max_tokens 两种可能。
